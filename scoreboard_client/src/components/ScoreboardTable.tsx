...
function ScoreboardTable({ teams }: ScoreboardTableProps) {
  const dispatch = useAppDispatch();
  const correctQuestions = useAppSelector((state) => state.correctQuestions);
  ...
  return teams.length > 0 ? (
      ...
      <table className="w-full text-left">
        <thead className="text-base uppercase dark:text-white dark:bg-neutral-700 text-black bg-gray-50">
          <tr>
            ...
            <th className="py-3 px-6">RP</th>
            <th className="py-3 px-6 hidden lg:table-cell"># Correct</th>
            <th className="py-3 px-6 hidden lg:table-cell">Geantwoord</th>
            <th className="py-3 px-6">Antwoord</th>
          </tr>
        </thead>
        <tbody>
          {teams.map((team, index) => (
            <tr
              key={team._id}
              className="bg-white dark:bg-neutral-600 dark:border-neutral-400 border-b"
            >
              <td className="py-4 px-6 font-black">{index + 1}</td>
              <td className="py-4 px-6 dark:text-white dark:font-bold text-gray-900 whitespace-nowrap">
                {team.name}
              </td>
              <td className="py-4 px-6">
                <span className="bg-indigo-500 py-px text-white rounded-full px-4">
                  {team.roundPoints}
                </span>
              </td>
              <td className="py-4 px-6 hidden lg:table-cell">
                {correctQuestions &&
                correctQuestions.find(
                  (correctQuestion) => correctQuestion.teamId === team._id,
                ) ? (
                  <p>{`${correctQuestions.find((correctQuestion) => correctQuestion.teamId === team._id)?.correctAnswers} deze ronde`}</p>
                ) : (
                  <p>0 deze ronde</p>
                )}
              </td>
              <td className="py-4 px-6 hidden lg:table-cell">
                {askedQuestion &&
                askedQuestion.givenAnswers.find(
                  (givenAnswer) => givenAnswer.team._id === team._id,
                ) ? (
    ...
    <div className="flex flex-col gap-y-10 items-center justify-center w-full h-full">
      ...
      <h2 className="text-neutral-600 dark:text-gray-300">
        Wachten op teams. Join via de bovenstaande url. De quizmaster zal dan
        oordelen of je deel mag nemen.
      </h2>
    </div>
  );
}
