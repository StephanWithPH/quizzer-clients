    ...
    <DashboardLayout>
      <div className="flex flex-col gap-y-10">
        ...
        <div className="grid xl:grid-cols-3 divide-y xl:divide-none divide-neutral-500 gap-10">
          <div className="flex flex-col gap-y-4 p-6 overflow-y-auto max-h-[33rem]">
            {quiz?.teams?.length > 0 ? (
              quiz?.teams.sort((a, b) => b.roundPoints - a.roundPoints).map((team, index) => (
                <div key={team._id} className="flex items-center gap-10 justify-between">
              ...
              <div className="text-sm w-full h-full border border-dashed rounded-md dark:border-neutral-500
                border-gray-500 my-6 p-2 inline-flex items-center justify-center"
              >
                Geen Teams
              </div>
            )}
          </div>
          <div className="flex flex-col gap-y-4 items-center py-10 xl:py-0">
